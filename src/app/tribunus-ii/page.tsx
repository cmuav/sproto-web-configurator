...
// ── Enum labels (Tribunus II specific) ───────────────────────────────────────
...
const ENUM_LABELS: Record<string, Record<number, string>> = {
  ...
  BecVoltages: { 0: "5.1V", 1: "6.1V", 2: "7.3V", 3: "8.3V", 4: "Disabled" },
...
function readProp(data: Uint8Array, p: PropertyDef): number | string {
  ...
  const raw = readRaw(data, p.offset, byteSize(p.type, p.size));
  switch (p.type) {
...
const SYSTEM_NAMES: Record<string, string> = {
  activeTime: "Active Time", serialNumber: "Serial Number", escType: "ESC Type",
  bootloaderVersion: "Bootloader", firmwareVersion: "Firmware",
};

const STATE_NAMES: Record<string, string> = {
  throttle: "Throttle", motorRPM: "Motor RPM", supplyVoltage: "Battery Voltage",
  current: "Current", cpuTemperature: "CPU Temp", mosfetTemperature: "MOSFET Temp",
  outputPower: "Output Power", becVoltage: "BEC Voltage", powerConsumption: "Consumption",
};

const SETTINGS_NAMES: Record<string, string> = {
  deviceName: "Device Name", mode: "Device Mode", becVoltage: "BEC Voltage",
  rotationDirection: "Rotation", protocol: "Protocol",
  startTime: "Start Time", rampTime: "Ramp Time", bailoutTime: "Bailout Time",
  pGain: "P Gain", iGain: "I Gain", dGain: "D Gain", smooth: "Smooth", storedRpm: "Stored RPM",
  ...
  soundConfiguration: "Sound Volume", sensitivityGain: "Sensitivity Gain",
  dynamicProtection: "Dynamic Protection", rpmCorrection: "RPM Correction",
  throttleMaxCal: "Max Throttle", throttleZeroCal: "Zero Throttle", throttleMinCal: "Min Throttle",
};

// ── Component ────────────────────────────────────────────────────────────────

export default function TribunusIIPage() {
  ...
  function isDisconnectError(e: unknown): boolean {
    ...
    return msg.includes("disconnected") || msg.includes("device has been lost") || msg.includes("EPIPE") || msg.includes("Port closed") || msg.includes("Port is not");
  }

  function handleLost() {
    setLoading(false); setLoadingLabel("");
    setError("Device disconnected. Reconnect the USB cable and try again.");
    setState("lost");
  ...
  const reconnect = useCallback(async () => { await doDisconnect(); setError(null); await connect(); }, [connect]); // eslint-disable-line react-hooks/exhaustive-deps
  const disconnect = useCallback(async () => { await doDisconnect(); setState("disconnected"); setSystemData(null); setStateData(null); setSettingsData(null); setOrigSettings(null); setError(null); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const readAll = useCallback(async () => {
    ...
    try {
      ...
      setSystemData(trib.regions.system.readRaw(0, trib.regions.system.byteLength));

      setLoadingLabel("Reading state...");
      await trib.device.readRegion(trib.regions.state, undefined, undefined, (p) => setProgress(20 + p * 0.2));
      setStateData(trib.regions.state.readRaw(0, trib.regions.state.byteLength));
      ...
      const sd = trib.regions.settings.readRaw(0, trib.regions.settings.byteLength);
  ...
  const writeSettings = useCallback(async () => {
    const trib = tribRef.current; if (!trib || !settingsData) return;
  ...
  const refreshState = useCallback(async () => {
    ...
    try {
      ...
      setStateData(trib.regions.state.readRaw(0, trib.regions.state.byteLength));
    } catch (e: unknown) { if (isDisconnectError(e)) handleLost(); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    <div className="min-h-full bg-background">
      <div className="sticky top-0 z-10 bg-card/80 backdrop-blur border-b border-border px-4 py-3">
        <div className="flex items-center gap-3 max-w-2xl mx-auto">
          <Link href="/" className="p-2 -ml-2 rounded-lg hover:bg-muted active:bg-muted/80 transition-colors"><ArrowLeft className="h-5 w-5" /></Link>
          <h1 className="text-base font-bold flex-1">Tribunus II ESC</h1>
      ...
      <div className="p-4 max-w-2xl mx-auto flex flex-col gap-4">
        {!webUsbSupported && <div className="rounded-xl bg-red-50 border border-red-200 p-4"><p className="text-sm font-semibold text-red-700">WebUSB not supported. Use Chrome or Edge.</p></div>}

        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center gap-3 mb-4">
            <Zap className="h-5 w-5 text-muted-foreground" />
            <div>
              <p className="text-sm font-semibold">Tribunus II / II+</p>
          ...
          {state === "disconnected" ? (
            <Button className="w-full h-12 text-base font-semibold rounded-xl" onClick={connect} disabled={!webUsbSupported}><Usb className="h-4 w-4 mr-2" /> Connect</Button>
          ) : state === "connecting" ? (
            <Button className="w-full h-12 rounded-xl" disabled>Connecting...</Button>
          ) : state === "lost" ? (
            <div className="flex gap-2">
              <Button className="flex-1 h-12 text-base font-semibold rounded-xl" onClick={reconnect}><Usb className="h-4 w-4 mr-2" /> Reconnect</Button>
              <Button variant="outline" className="h-12 rounded-xl text-destructive border-destructive/30" onClick={disconnect}><Unplug className="h-4 w-4" /></Button>
            </div>
          ) : (
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 h-10 rounded-xl" onClick={readAll} disabled={loading}><Download className="h-4 w-4 mr-2" /> {settingsData ? "Re-read" : "Read"}</Button>
              {hasChanges && <Button className="flex-1 h-10 rounded-xl" onClick={writeSettings} disabled={loading}><Upload className="h-4 w-4 mr-2" /> Write</Button>}
              <Button variant="outline" className="h-10 rounded-xl text-destructive border-destructive/30" onClick={disconnect}><Unplug className="h-4 w-4" /></Button>
            </div>
          )}
          {loading && <div className="mt-3"><div className="flex justify-between mb-1"><p className="text-xs text-muted-foreground">{loadingLabel}</p><p className="text-xs text-muted-foreground tabular-nums">{Math.round(progress)}%</p></div><div className="h-1.5 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full transition-all" style={{ width: `${progress}%` }} /></div></div>}
        ...
        {stateData && (
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button variant="ghost" size="sm" className="h-7 px-2" onClick={refreshState}><RefreshCw className="h-3.5 w-3.5" /></Button>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {STATE_ENTRIES.map(({ key, name, def }) => {
                const val = readProp(stateData, def);
                const display = typeof val === "number" && (key === "throttle" || key === "outputPower")
                  ? (val * 100).toFixed(1) : formatProp(val, def, key);
                return (
                  <div key={key} className="rounded-lg bg-muted/30 px-3 py-2">
                    <p className="text-[10px] text-muted-foreground">{name}</p>
                    <p className="text-sm font-semibold font-mono">{display}{def.unit ? <span className="text-xs text-muted-foreground ml-1">{def.unit}</span> : null}</p>
                  </div>
                );
              })}
            </div>
        ...
        {settingsData && (() => {
          const GROUP_LABELS: Record<string, string> = { main: "General", heli: "Helicopter / Governor", plane: "Airplane", protection: "Protection", configuration: "Configuration" };
          let lastGroup = "";
          return (
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex flex-col gap-1.5">
                {SETTINGS_ENTRIES.map((entry) => {
                  ...
                  return (
                    <div key={entry.key}>
                      {showHeader && group && <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mt-3 mb-1.5 px-1">{GROUP_LABELS[group] ?? group}</p>}
...
function SettingRow({ entry, data, original, onChange }: { entry: PropEntry; data: Uint8Array; original: Uint8Array; onChange: (data: Uint8Array) => void }) {
  ...
  function commit() {
    if (options) { const opt = options.find((o) => o.label === input); if (opt) { onChange(writeProp(data, def, String(opt.value))); setEditing(false); return; } }
    onChange(writeProp(data, def, input)); setEditing(false);
  }

  return (
    <div className={`flex items-center gap-3 py-2 px-3 rounded-lg ${modified ? "bg-amber-50 border border-amber-200" : "bg-muted/20"}`}>
      <div className="flex-1 min-w-0"><p className="text-xs font-medium">{name}</p></div>
      {editing ? (
        options ? (
          <select className="text-xs font-mono bg-background border rounded px-2 py-1" autoFocus value={input} onChange={(e) => setInput(e.target.value)} onBlur={commit}>
            {options.map((o) => <option key={o.value} value={o.label}>{o.label}</option>)}
          </select>
        ) : (
          <Input className="w-28 h-7 text-xs font-mono text-right" autoFocus value={input} onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") commit(); if (e.key === "Escape") setEditing(false); }} onBlur={commit} />
        )
      ) : (
        <button
          className={`text-xs font-mono font-medium px-2 py-1 rounded min-w-[60px] text-right ${def.readOnly ? "text-muted-foreground bg-muted/30" : "bg-muted/50 hover:bg-muted"} ${modified ? "text-amber-700" : ""}`}
          onClick={() => { if (!def.readOnly) { setInput(currentDisplay); setEditing(true); } }} disabled={def.readOnly}
        >
